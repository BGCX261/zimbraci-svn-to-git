...
static string get_pref_string ( const char *pref_key, gchar *def_val=(gchar *)"N/A") {
	GConfEngine *gce = gconf_engine_get_default();
	string pref_val("zimbraci");
	char * str = gconf_engine_get_string(gce,(string("/zimbraci/")+pref_key).c_str(),NULL);
	pref_val = string(str?str:"");
	Log((string("pref_val inited.")+pref_val).c_str());
...
static void auth_complete(SoupSession *session, SoupMessage *msg, gpointer user_data) {
	...
	if (!authToken.empty()) {
        ...
        soup_session_queue_message( session, sm, query_complete, NULL);
    }
}
...
int main( int argc, char *argv[]) {
	...
	Log("Loading conf...");
	if (get_pref_string("url")=="N/A" ||
		get_pref_string("usr")=="N/A" ||
		get_pref_string("psw")=="N/A" ||
		get_pref_string("tmo")=="N/A" ||
		get_pref_string("qry")=="N/A")
		prefs_popup();
	else
		timeout_id = g_timeout_add_seconds( interval, (GtkFunction) check_zimbra, NULL);
